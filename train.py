...
from Agent.TD3Agent import TD3Agent
...
##################################### Training config
parser.add_argument('--agent_type', default='DDPG', type=str, help='Type of agent for training')
...
parser.add_argument('--actor_delay_freq', default=2, type=int, help='Actor delay update for TD3 Agent')
...
parser.add_argument('--retrain_model', default=0, type=int, help='If retrain pretrain model')
...
########## use gru to process history info(state / action)
...
parser.add_argument('--use_topology_info', default=0, type=int, help='If use gcn to process topology structure')
parser.add_argument('--feature_num', default=4, type=int, help='Number of features for each node')
parser.add_argument('--gcn_hidden_size', default=32, type=int, help='Number of features for each node')
parser.add_argument('--gcn_dropout', default=0.1, type=float, help='Drop out rate for gcn layers')
########## use for sampling data while training
parser.add_argument('--sample_by_train', default=0, type=int, help='If sample while train')
parser.add_argument('--data_block_size', default=500000, type=int, help='Size of data in a block')
parser.add_argument('--min_episode_length', default=30, type=int, help='Min episode length for saving')
parser.add_argument('--sample_data_dir', default='./sample_data/episode_data/', type=str, help='Dir of data to save')
########## model structure
...
if __name__ == "__main__":
    ...
    if config.agent_type == 'DDPG':
        ...
        my_agent = A2CAgent(settings, config)
    elif config.agent_type == "TD3":
        my_agent = TD3Agent(settings, config)
    else:
