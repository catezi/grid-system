...
class DDPG:
    def __init__(self, settings, config):
        ...
        self.critic = Critic(settings, config).to(self.device)
        self.critic_target = Critic(settings, config).to(self.device)
    ...
    def update(self, f_learner, state_norm):
        ...
        if self.config.use_mini_batch:
            for index in BatchSampler(SubsetRandomSampler(range(self.config.batch_size)), self.config.mini_batch_size, False):
                """
                    mini_states torch.Size([mini_batch_sze, state_dim])
                    mini_actions torch.Size([mini_batch_sze, action_dim])
                    mini_rewards torch.Size([mini_batch_sze, 1])
                    mini_dones torch.Size([mini_batch_sze, 1])
                    mini_next_states torch.Size([mini_batch_sze, state_dim])
                """
    ...
    def load(self, state_norm=None):
        if self.config.load_model or self.config.retrain_model:
            ...
            self.actor_optimizer.load_state_dict(save_model['actor_optimizer'])
            self.critic.load_state_dict(save_model['critic_network'])
            self.critic_target.load_state_dict(save_model['critic_network'])
            self.critic_optimizer.load_state_dict(save_model['critic_optimizer'])
...
class DDPGAgent(BaseAgent):
        ...
        for episode in range(self.config.max_episode):
            ...
            for timestep in range(self.config.max_timestep):
                ...
                if self.policy.get_buffer_size() >= self.config.batch_size and \
                    ...
                    self.policy.update(self.f_learner, self.state_norm)
    ...
    def evaluate(self, process_idx,speed):
        set_seed(123)
        rounds = []
        rtgs = []
        # load pretrain model(and state normal) if enable
        ...
        for _, episode in evaluate_tqdm:
            ...
            for timestep in range(self.config.max_timestep):
                ...
                cursor = db.cursor()
                #观察数据是否少于一千条
                sql = "select count(*) from grid_powergrid"
                cursor.execute(sql)
                result = cursor.fetchone()
                if result[0] < 1000:
                    sql = "INSERT INTO  grid_powergrid (\
                                        vTime,\
                                        gen_p,\
                                        gen_q,\
                                        gen_v,\
                                        ...
                                        reward ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                    val = (
                        str(next_obs.vTime),
                        str(next_obs.gen_p),
                        str(next_obs.gen_q),
                        str(next_obs.gen_v),
                        str(next_obs.target_dispatch),
                        str(next_obs.actual_dispatch),
                        str(next_obs.ld_p),
                        str(next_obs.adjld_p),
                        str(next_obs.stoenergy_p),
                        str(next_obs.ld_q),
                        str(next_obs.ld_v),
                        str(next_obs.p_or),
                        str(next_obs.q_or),
                        ...
                        str(next_obs.v_ex),
                        str(next_obs.a_ex),
                        str(next_obs.line_status),
                        str(next_obs.grid_loss),
                        str(next_obs.bus_v),
                        str(next_obs.bus_gen),
                        str(next_obs.bus_load),
                        str(next_obs.bus_branch),
                        str(next_obs.flag),
                        str(next_obs.unnameindex),
                        str(next_obs.action_space),
                        str(next_obs.steps_to_reconnect_line),
                        str(next_obs.count_soft_overflow_steps),
                        str(next_obs.rho),
                        str(next_obs.gen_status),
                        str(next_obs.steps_to_recover_gen),
                        str(next_obs.steps_to_close_gen),
                        str(next_obs.curstep_renewable_gen_p_max),
                        str(next_obs.nextstep_renewable_gen_p_max),
                        str(reward))
