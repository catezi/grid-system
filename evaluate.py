...
parser.add_argument('--quick_tqdm', default=1, type=int, help='If use high frequency for tqdm')
parser.add_argument('--num_process', default=4, type=int, help='Num process for evaluation')
...
parser.add_argument('--speed', default=1, type=int, help='simulate speed')
parser.add_argument('--policy', default='DDPG', type=str, help='simulate speed')

def start_evaluate(process_idx):
    ...
    speed=config.speed
    my_agent.evaluate(process_idx,speed)
...
if __name__ == "__main__":
    ...
    if(policy=='DDPG'):
        barrier = threading.Barrier(config.num_process + 1)
        for i in range(config.num_process):
            actor = threading.Thread(
                target=start_evaluate,
                args=(i, ),
            )
            actor.start()
            all_actor_process.append(actor)
    elif(policy=='random'):
        my_agent = RandomAgent(settings, config)
        speed = config.speed
        my_agent.evaluate(0, speed)
